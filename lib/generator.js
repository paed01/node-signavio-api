...
function generate(apiName, apiDef, options) {
  ...
  internals.Interface = function(ioptions) {
    ...
    this.options.basePath = (ioptions && ioptions.basePath) || apiDef.basePath;
    ...
    const logFunction = (ioptions && ioptions.log) || options.log; // && typeof ioptions.log === 'function' && ioptions.log;
    if (logFunction) {
  ...
  internals.Interface.prototype.proxy = function(reqOptions, callback) {
    const self = this;
    const roptions = Object.assign({}, reqOptions);
